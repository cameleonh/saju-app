...
const matchPoolPhoto = await fetch(`${url}/images/matches/pool/wood_female_01.jpg`);
assert.equal(matchPoolPhoto.status, 200);
assert.equal(matchPoolPhoto.headers.get('content-type'), 'image/jpeg');
assert.equal((await fetch(`${url}/images/matches/pool/manifest.json`)).status, 404, 'pool provenance stays out of the public allowlist');
...
assert.deepEqual(natalPolicyBody.calculationPolicy, { id: 'KR-CIVIL-1.0', version: '1.2.0', engine: 'gyeol-natal-core', engineVersion: '1.2.0' });

const annualOutOfRange = await fetch(`${url}/v1/annual-readings`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ targetYear: 2100, natal: { dayStem: '戊', monthBranch: '戌', branches: ['午', '戌', '申', '未'] }, chartPolicy: { id: 'KR-CIVIL-1.0', version: '1.2.0', engine: 'gyeol-natal-core', engineVersion: '1.2.0' } }) });
