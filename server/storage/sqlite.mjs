...
export function createSqliteStorage(filePath = ':memory:') {
  ...
  return {
    ...
    saveSubmission({ submissionId, input, projection, status }) {
      insert.run(submissionId, input.clientRequestId, input.relationshipMode || 'single', JSON.stringify(input.dataSubject), input.partnerSubject ? JSON.stringify(input.partnerSubject) : null, JSON.stringify(input.birthInput), input.sourceBirthInput ? JSON.stringify(input.sourceBirthInput) : null, input.partnerBirthInput ? JSON.stringify(input.partnerBirthInput) : null, input.partnerSourceBirthInput ? JSON.stringify(input.partnerSourceBirthInput) : null, JSON.stringify(input.chartResult), JSON.stringify(input.purposeReceipts), input.partnerPurposeReceipts ? JSON.stringify(input.partnerPurposeReceipts) : null, projection ? JSON.stringify(projection) : null, status, new Date().toISOString());
    },
