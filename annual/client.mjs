...
export function privacySafeAnnualExport(annual) {
  ...
  return {
    ...
    effectiveRange: annual.effectiveRange, timezone: annual.timezone, calculationPolicy: annual.calculationPolicy,
    interpretationProfile: annual.interpretationProfile, ruleSet: annual.ruleSet, facts: annual.facts, cards: annual.cards,
    monthlyFlow: annual.monthlyFlow, contentHash: annual.contentHash,
...
function monthlyMarkup(month) {
  if (month.status === 'unsupported') return `<li><strong>${escapeHtml(month.label)}</strong><p>경계 자료를 지원하지 않아 월운 해석을 만들지 않았습니다.</p></li>`;
  return `<li><article><div class="monthly-head"><strong>${escapeHtml(month.label)} · ${escapeHtml(month.pillar)}</strong><span>${escapeHtml(formatRange(month.effectiveRange))}</span></div><p>${escapeHtml(month.theme)}</p><p><strong>활용:</strong> ${escapeHtml(month.use)}</p><p><strong>주의:</strong> ${escapeHtml(month.watch)}</p></article></li>`;
}
