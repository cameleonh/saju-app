...
function domainMarkup(domain, domainNumber) {
  const points = (domain.points || []).map((p) => `<li>${escapeHtml(p)}</li>`).join('');
  return `<article class="reading-domain-card" data-domain-key="${escapeHtml(domain.domain_key)}"><h4><span class="domain-number" aria-hidden="true">${String(domainNumber).padStart(2, '0')}</span>${escapeHtml(domain.domain_label)}</h4><ol>${points}</ol>${domain.closing ? `<p class="domain-closing">${escapeHtml(domain.closing)}</p>` : ''}</article>`;
}

function sortedDomains(domains) {
  return [...domains].sort((a, b) => (Number(a.domain_index) || 99) - (Number(b.domain_index) || 99) || String(a.domain_key).localeCompare(String(b.domain_key)));
}

// 근거→해석 포장 조각들 — 모두 결정론 템플릿(숫자 점수 없음, LLM 사용 없음)
function buildAnnualPackaging(annual, elementCounts) {
  const byId = new Map((annual.facts || []).map((item) => [item.id, item]));
  const tenGodFact = byId.get('annual.stem.tenGodToDayMaster');
  const relationsFact = byId.get('annual.branch.relationsToNatal');
  const pillarFact = byId.get('annual.year.pillar');
  const relations = Array.isArray(relationsFact?.value) ? relationsFact.value : [];
  const badge = flowBadge({ tenGod: tenGodFact?.value, relations });
  const relationSummary = relations.map((item) => `${item.natalPosition}:${item.natalBranch} ${item.relation === 'clash' ? '충' : '육합'}`).join(' · ');
  const flow = evidenceFlowMarkup([
    pillarFact ? { label: '연운 간지', value: pillarFact.value } : null,
    tenGodFact ? { label: '일간과의 십신', value: tenGodFact.value } : null,
    relationSummary ? { label: '원국 지지와의 관계', value: relationSummary } : null,
  ].filter(Boolean), '8장 카드와 13항목 풀이로 이어집니다');
  const dayStem = tenGodFact?.detail?.match(/일간 (\S)을/)?.[1] || null;
  const remark = saeseongRemark({ dayStem, badge, period: { kind: 'annual', label: `${annual.yearPillar}년` } });
  const quest = buildQuest({
    candidates: [
      annual.cards.find((card) => card.cardType === 'action')?.action,
      (annual.domains || []).find((domain) => domain.domain_key === 'must_do')?.points?.[0],
      annual.cards.find((card) => card.action)?.action,
    ],
  });
  const lucky = elementCounts ? luckyElementProps(elementCounts) : null;
  const packRow = `<div class="pack-row">${badgeMarkup(badge, `${annual.targetYear}년 흐름`)}${questMarkup(quest)}</div>`;
  return { flow, packRow, remarkBlock: remarkMarkup(remark, `${annual.targetYear}년 연운`), luckyBlock: lucky ? luckyPropsMarkup(lucky) : '' };
}

export function renderAnnualReading(annual, { activeIndex = 0, activeFact = '', loading = false, error = '', elementCounts = null } = {}) {
