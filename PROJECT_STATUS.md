...
2. **Tử Vi placement rewritten and oracle-verified** — Ming/Than palaces via the orthodox month-forward/hour-wrapping procedure, bureau (Ngũ Hành Cục) via the nayin five elements of the Ming stem-branch with five-tiger-dun month stems, Zi Wei placement by the canonical quotient/difference rule, and the 14 major stars through the Zi Wei group (retrograde offsets) and Tian Fu group (direct offsets). Cross-checked against the independent Vietnamese library `tuvi-neo@1.0.7`: 246 sampled charts (1936–2004) agree on lunar date, Ming palace, bureau, and every star; 6 residual cases are known Chinese-vs-Vietnamese calendar boundary months (1984-11, 1996-07), which the policy already discloses as the Chinese-calendar substitute limitation. `tests/unit/tu-vi.mjs` pins the oracle fixtures. **Tứ hóa added (2026-08-27):** the four transformations follow the Quanshu-lineage table (verified per host star and palace on 213 charts, zero mismatches) with the four tu-hoa auxiliary stars (Văn Xương/Khúc by hour branch, Tả Phù/Hữu Bật by lunar month). **Minor stars added (2026-08-27):** all fourteen classical Phụ tinh placed by year stem, year branch (trio + yin/yang for Hỏa/Linh), hour branch, and lunar month — 695 sampled charts × 14 stars match the oracle with zero mismatches. **Đại hạn/Tiểu hạn added (2026-08-27):** decade limits start at the bureau-number age and advance one palace per decade forward for yang-year males / yin-year females and backward otherwise; yearly limits walk one palace per nominal age (male backward). The tuvi-neo oracle leaves these cycles unimplemented, so the rule follows multiple corroborating Chinese references with hand-verified fixtures (37-nominal 1990 fixture: Đại hạn 35–44 at Tỵ, Tiểu hạn at Mệnh) and boundary checks at ages 4/5/14/15. Remaining gap: none for placement; 소한 female-direction variant assumes the common school default.
3. **Horasat rasi realigned** — Boundaries now follow the Thai Wikipedia chankrasi sidereal (นิรายนะ) table with before/exact/after boundary tests. **Jupiter table extended (2026-08-27):** now spans 2024–2035, each year keyed to Jupiter's first DIRECT sidereal ingress (retrograde re-entries skipped), computed with astronomy-engine minus the Lahiri ayanamsa and cross-checked 4/4 against the previous rows; out-of-range years still return null and the card is omitted.
4. **Mahabote placement orthodoxized (2026-08-27)** — The house arithmetic was rewritten from the dirah.org mahabote lesson (acquired in full): Burmese-era year by the April 15 split, year mod 7 fixing the first-house planet (1 Sun … 0 Saturn), houses filled in the mahabote planet order (Sun-Mercury-Saturn-Mars-Venus-Moon-Jupiter), birth-weekday planet (Wednesday PM = Rahu) marking the person's house. The 1985-02-20 worked example reproduces the printed chart exactly; the prior code had misused the remainder as a weekday index. Annual-cycle interpretation (house + age mod 7) remains a common-practice reading, and the UI/labels now state that placement arithmetic is source-verified while interpretations stay reference-grade. Tiểu hạn direction is fixed to the male default (no gender collected) and documented.
