...
- Use `KR-CIVIL-1.0`: Korean legal civil time, IANA `Asia/Seoul` 2026c historical transitions, exact minute `jie` boundaries, civil-midnight day rollover, `23:00–00:59` Zi hour, no hidden longitude/solar correction. Daewoon is supported under the separate `KR-DAEWOON-1.0` policy.
...
- Navigation now has distinct method and data screens, active current-page states, and a stage contract that restores heading focus at scroll position zero. The service worker uses cache `saju-app-shell-v11`, precaches the annual modules, natal engine/ephemeris, and daewoon engine, retains network-first navigation and a canonical offline shell fallback, and does not eagerly precache the birthplace catalog.
...
- The one-time global Apache `ServerName 127.0.0.1` configuration was enabled on Lightsail. Two fresh `apache2ctl configtest` runs returned `Syntax OK` without `AH00558`; Apache, the Node service, and the durable SQLite health endpoint remained active after reload. The initially documented `saju.blog` global value was rejected after verification showed that Debian's unnamed `default-ssl.conf` inherited it and served its self-signed certificate for the same SNI name. External verification after correction returned the Let's Encrypt `CN=saju.blog` certificate with `ssl_verify_result=0`. `DEPLOYMENT.md` records the safe repeatable procedure and collision rationale.
...
- Daewoon (major cycle) direction, start age, and pillar sequence are calculated under `KR-DAEWOON-1.0@1.0.0` but interpretation (용신·격국·신강신약·조후·대운×연운 교차 해석) is intentionally unsupported.
- Google Fonts (`Noto Sans KR`, `Noto Serif KR`) is the primary font source. The fallback chain includes Linux CJK fonts (`Noto Sans CJK KR`, `Nanum Gothic`, `Droid Sans Fallback`), but environments without any Korean font installed will show missing-glyph boxes when Google Fonts is unreachable. Self-hosting Korean web fonts is the only complete fix but is deferred due to file size (~1MB+ per weight).
...
- Daewoon interpretation (용신·격국·신강신약·조후·대운×연운 교차), longitude or apparent-solar correction, overseas birthplaces, and school-specific strength/yongsin/gyeokguk rules remain deliberately unsupported rather than inferred.
