...
// 베트남 Tử Vi chart engine. Structural placement rules are shared with the
// selected Quanshu lineage; lunar-date conversion follows the VN GMT+7 calendar.

import { describeSolarToVietnameseLunar } from './vietnamese-lunar-calendar.mjs';
import { resolveSeoulCivilTime } from './natal-engine.mjs';

export const TU_VI_POLICY = Object.freeze({
  id: 'VN-TUVI-1.0',
  version: '1.2.0',
  name: '베트남 현지 음력·자미두수 배치 정책',
  source: '음력은 Hồ Ngọc Đức의 베트남 GMT+7/105E 천문력 규칙을 사용한다. 명궁·신궁·국·자미 14주성·사화·잡성 배치는 Quanshu 계열 규칙과 tuvi-neo 1.0.7 대조값을 사용한다. 윤달은 같은 번호의 평월 규칙으로 처리한다. 대·소한 진행 방향에는 전통 계산용 성별값이 필요하다.',
  calendarSource: 'ho-ngoc-duc-vietnamese-lunar-calendar@2008-rules-gmt+7.1',
  calendarTimezone: 'Asia/Ho_Chi_Minh (UTC+7)',
  hourRule: 'birthplace civil clock; exact HH:MM required',
...
export function getDaiHan(chart, age) {
  if (!['male', 'female'].includes(chart.sex)) return null;
  ...
  const forward = (isYangYear && chart.sex === 'male') || (!isYangYear && chart.sex === 'female');
...
export function getTieuHan(chart, age) {
  ...
  // 통용 규칙: 남명 역행·여명 순행, 명궁을 1세로 1년 1궁.
...
export function calculateTuVi(input = {}) {
  ...
  if (inputDate.getUTCFullYear() !== inputYear || inputDate.getUTCMonth() !== inputMonth - 1 || inputDate.getUTCDate() !== inputDay) {
    throw new Error('Tử Vi birth date must be a valid Gregorian date.');
  }
  const birthYear = Number(dateStr.slice(0, 4));
  if (birthYear < 1900 || birthYear > 2100) throw new Error('Tử Vi birth date must be within 1900..2100.');

  if (input.unknownTime === true || !input.time) throw new Error('Tử Vi requires an exact birth time to place the Life and Body palaces.');
  ...
  const utcMs = input.utcInstant ? Date.parse(input.utcInstant) : resolveSeoulCivilTime(dateStr, input.time).utcMs;
  if (!Number.isFinite(utcMs)) throw new Error('utcInstant must be a valid ISO timestamp');
  // Convert the birth instant to the Vietnamese calendar's GMT+7 day before
  // looking up its lunar date. The two-hour branch still uses the entered
  // birthplace civil clock, as required by the chosen chart policy.
  const vietnamLocal = new Date(utcMs + 7 * 60 * 60 * 1000);
  const vietnamDate = `${vietnamLocal.getUTCFullYear()}-${String(vietnamLocal.getUTCMonth() + 1).padStart(2, '0')}-${String(vietnamLocal.getUTCDate()).padStart(2, '0')}`;
  const lunar = describeSolarToVietnameseLunar({ date: vietnamDate });
  const lunarMonth = lunar.month; // 윤달은 같은 번호의 평월로 처리하는 정책
  const lunarDay = lunar.day;

  const [hours] = input.time.split(':').map(Number);
  const hourBranchIdx = getHourBranchIndex(hours);
  ...
  return {
    policy: TU_VI_POLICY,
    sex: ['male', 'female'].includes(input.sex) ? input.sex : null,
    unsupportedStates: [
      ...(lunar.leapMonth ? [{ id: 'tu-vi.leap-month', reason: 'This policy maps a leap lunar month to the same-numbered regular month.' }] : []),
      ...(!['male', 'female'].includes(input.sex) ? [{ id: 'tu-vi.gendered-periods', reason: 'Traditional sex parameter is required to calculate Đại Hạn and Tiểu Hạn direction.' }] : []),
    ],
...
export function calculateTuViAnnual(input = {}) {
  ...
  return {
    ...
    unsupportedStates: [
      { id: 'tu-vi.annual-interpretation', reason: '궁·별의 위치는 계산했지만 연도별 사건 해석 문안은 검증된 규칙이 없어 생성하지 않습니다.' },
      ...chart.unsupportedStates,
    ],
    summary: `${targetYear}년(${yearBranchInfo.name})의 유년 지지 ${BRANCH_NAMES_VN[yearBranchInfo.branchIdx].name}가 ${activePlacement.branch.name}의 ${activePalace.name}궁과 만납니다. 이는 배치 사실이며 사건 예측이 아닙니다.`,
  };
}

/**
 * No source-locked Vietnamese Nhật Vận rule is included in this policy.
 * @returns {null} Daily output is withheld until a lineage-specific rule is approved.
 */
export function calculateTuViDaily(input = {}) {
  // This repository has no source-locked Vietnamese Nhật Vận palace-rotation
  // rule. A lunar-day modulo-12 heuristic must not be presented as Tử Vi output.
  return null;
}
