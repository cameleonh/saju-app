...
> 문서 상태: Draft v1.0
> 작성일: 2026-08-23; calculation scope revision: 2026-09-24
> 대상 제품: `_saju-app`의 개인용 다전통 점성술 확장
> 구현 기준: 기존 바닐라 HTML/CSS/JavaScript 구조와 `DESIGN.md`, `DESIGN-SYSTEM.md`를 유지한다.
> 현재 상태: 네 개의 제한된 출생 차트 계산기가 런타임에 있다. 태국 상승점 전체 차트와 출처가 잠기지 않은 일부 연·일운 문안은 계산 범위 밖이다.
...
- 한국 사주는 `KR-CIVIL-1.0@1.2.0` 법정 민간시 정책을 기준으로 한다.
...
### 1.4 현재 정책의 계산 범위 밖에 있는 것

- 태국 호라삿의 상승점과 전체 행성·하우스 차트
- 출처가 고정되지 않은 Tử Vi 일운 회전 및 예측 문안
- 출처가 고정되지 않은 Mahabote 연운·일운 예측
- 근거 없는 네 체계 종합 예측
- 해외 출생지 및 과거 시각대 해석
- 다전통 결과 저장·공유·유료화

각 활성 엔진은 연결된 정책 문서에 적힌 제한 범위만 계산한다. 근거가 잠기지 않은 하위 기능은 `null` 또는 `unsupported`로 처리하고 실제 운세처럼 보이는 대체 문구를 반환하지 않는다.
...
`horasat.kr`을 포함한 상용 서비스의 출력은 탐색용 교차 확인 자료가 될 수 있지만 단독 오라클이 될 수 없다. 현재 정책 범위를 넘어 새로운 유파·기능을 활성화하려면 18장의 출처·오라클 게이트를 별도로 통과해야 한다.
...
| `horasat` | 태국 요일·태양라시 | Thai Horasat subset | `active` | 정확한 시각, 요일표, Lahiri 태양 진입 |
| `tu-vi` | 베트남 뜨비 | Tử Vi Đẩu Số | `active` | 베트남 GMT+7 음력; 정확한 시각 필요 |
| `mahabote` | 미얀마 마하보테 | Mahabote natal chart | `active` | BE 나머지·요일·7하우스 natal 산식 |
...
```text
KR-CIVIL-1.0 @ 1.0.0
TH-HORASAT-1.0 @ 1.2.0
VN-TUVI-1.0 @ 1.2.0
MM-MAHABOTE-1.0 @ 1.2.0
```
...
| 호라삿 subset | 유효한 출생일 | 정확한 시각, Asia/Seoul resolved instant | 계산 불가 | 요일 표식·Lahiri 태양라시; 전체 Lagna 차트 아님 |
| 뜨비 | 유효한 출생일 | 정확한 시각, 베트남 GMT+7 달력 변환; 성별값은 Đại/Tiểu Hạn에 필요 | 계산 불가 | 명반·연간 위치 사실; 일운 예측은 미제공 |
| 마하보테 | 유효한 출생일 | 수요일은 주간/라후 구분을 위한 시각 선택 | 수요일 시각 미상은 주·야 후보를 표시하고 그 밖의 출생 차트는 계산 가능 | natal BE·나머지·요일·7하우스; 연·일운 예측은 미제공 |
